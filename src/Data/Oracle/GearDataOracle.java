...
public class GearDataOracle {
	...
	public Boolean save(Vehicle vehicle, Map<Integer, Measure> gears) {
		try {
			...
			Iterator<Entry<Integer, Measure>> iterator = gears.entrySet().
				iterator();
			for (int i = 0; i < size; i++) {
				Entry<Integer, Measure> entry = iterator.next();
				param01[i] = 0;
				param02[i] = vehicle.getId();
				param03[i] = entry.getKey().toString();
			...
			for (int i = 0; i < size; i++) {
				System.out.println("GEAR INDEX: " + index[i]);
			}
			return true;
		} catch (Exception ex) {
			Error.
